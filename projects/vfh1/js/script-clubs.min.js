...
const createListFilterClubs = () => {
  let metro = form.querySelector('select[name="metroStation"]').value;
  let pool = form.querySelector('select[name="pool"]').value;
  let clubName = form.querySelector('select[name="clubName"]').value;
  let filterClubs = [];
  filterClubs = clubs.filter(club => {
    if (clubName == '') {
      return club.metro.toString().includes(metro) && club.pool == pool;
    } else {
      return (
        club.title == clubName &&
        club.pool == pool &&
        club.metro.toString().includes(metro)
      );
    }
  });
