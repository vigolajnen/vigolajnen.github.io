...
(function() {
  const location = () => {
    const city = document.querySelector('.nav-city__title');
    // создание блока
    let page = document.querySelector('.main-location');
...
const form = document.querySelector('#clubs-form-search');

const filterListClubs = document.getElementById('modal-filter-list-clubs');
const modalFilterListClubs = new bootstrap.Modal(filterListClubs);
const modalBodyList = document.querySelector(
  '.modal-filter-list-clubs .modal-body',
);
const modalBaseContentList = modalBodyList.querySelector('div');

// Создадим контент для превью.
let listItemClub = club => {
  let div = document.createElement('div');
  div.innerHTML =
    `
    <section class="card-club card-club--list-item">
      <div class="card-club__title">Fitness House <span>` + club.title + `</span>` + `</div>
      ...
      <a class="card-club__pic" href=` + club.href + `>
        <img src=` + club.poster + ` width="666" height="228px" alt="фитнес-клуб с&nbsp;большим бассейном">
        </a>
...
let sidebar = document.querySelector('.sidebar-clubs');
let sidebarBody = sidebar.querySelector('.sidebar-clubs__list-clubs');
let btnСlose = document.querySelector('[data-target="sidebar-list-close"]');
let formFilterBtn = document.querySelector('[data-target="sidebar-list"]');
let btnListMobile = document.querySelector(
  '[data-target="sidebar-list-modal"]',
);

export const createListFilterClubs = () => {
  let stations = form.querySelector('select[name="stations"]').value;
  let category = form.querySelector('select[name="category"]').value;
  let clubName = form.querySelector('select[name="clubs"]').value;
  let filterClubs = [];

  filterClubs = clubs.filter(club => {
    // default
    if (
      clubName == 'Выберите клуб' &&
      stations == 'Выберите метро' &&
      ...
      if (club.category === category) {
        return clubs.filter((item) => {item.category === category})
      }
      if (club.title === clubName) {
        return clubs.filter((item) => {item.title === clubName})
      }
      if (club.stations.toString().includes(stations)) {
        return clubs.filter((item) => {item.stations === stations})
      }
    }
  });
  ...
  if (filterClubs.length == 0) {
    console.log('Нет вариантов');
  } else if (window.matchMedia('(min-width: 768px)').matches) {
    for (var i = 0; i < filterClubs.length; i++) {
      sidebarBody.append(listItemClub(filterClubs[i]));
    }
  } else {
    // -----------------------------
    ...
    filterListClubs.addEventListener('hidden.bs.modal', function() {
      modalBodyList.removeChild(modalBaseContentList);
    });

    // ---------------------

    for (var i = 0; i < filterClubs.length; i++) {
      divList = listItemClub(filterClubs[i]);
      modalBodyList.append(divList);
    }
  }
};

const openListClubs = () => {
  createListFilterClubs();
  sidebar.classList.add('show');
};

const openListClubsModal = () => {
  createListFilterClubs();
  if (modalFilterListClubs) {
    modalFilterListClubs.show();
  }
};

const closeListClubs = () => {
  sidebar.classList.remove('show');
};

btnСlose.addEventListener('click', closeListClubs, false);

if (formFilterBtn && window.matchMedia('(max-width: 699px)').matches) {
  formFilterBtn.addEventListener('click', openListClubsModal, false);
  btnListMobile.addEventListener('click', openListClubsModal, false);
} else {
  formFilterBtn.addEventListener('click', openListClubs, false);
}
...
const clubs = [
  {
    ...
    category: 'Фитнес Хаус с бассейном',
  ...
  },
  {
    ...
    category: 'Фитнес Хаус Премьер',
    title: 'Premier в Охта Молле',
    stations: ['Новочеркасская'],
  ...
  },
  {
    ...
    title: 'на Камышовой',
    stations: ['Комендантский проспект', 'Старая деревня'],
  ...
  },
  {
    ...
    title: 'на Шаврова',
    stations: ['Комендантский проспект'],
  ...
  },
  {
    ...
    title: 'на Фучика',
    stations: ['Бухарестская', 'Международная'],
  ...
  },
  {
    ...
    category: 'Фитнес Хаус Престиж',
    stations: ['Звездная', 'Московская'],
  ...
  },
  {
    ...
    title: 'на Савушкина',
    stations: ['Беговая', 'Старая деревня'],
...
const listCategory = [
  ...
  'Фитнес Хаус Престиж',
  'Фитнес Хаус Премьер',
  'Фитнес Хаус с бассейном',
  'Фитнес Хаус без бассейна',
];
...
const listNameClubs = [
  'Выберите клуб',
  'Prestige на Гашека',
  'Prestige на Левашовском',
  'Prestige на Пулковском',
  'Prestige на Хошимина',
  'Premier в Охта Молл',
  'Premier на Салова',
  'на Большевиков',
...
const formSearch = document.querySelector('#clubs-form-search');
const labels = formSearch.querySelectorAll('.form-select');
const filtersAriaLabels = ['Выберите клуб', 'Выберите клуб', 'Выберите метро'];
const filtersName = ['category', 'clubs', 'stations'];
const filtersNameArr = [listCategory, listNameClubs, listStations];

const createOptionFilter = item => {
  return `<option value="` + item + `">` + item + `</option>`;
};

const filterOptions = arr => {
  for (let i = 0; i < arr.length; i++) {
    createOptionFilter(arr[i]);
  }
};

const createSelector = (title, arr, ariaLabel) => {
  const fieldFilter = document.createElement('select');
  const frag = new DocumentFragment();

  fieldFilter.classList.add('form-select');
  fieldFilter.classList.add('form-control');
  fieldFilter.setAttribute('name', title);
  fieldFilter.setAttribute('aria-label', ariaLabel);

  for (let i in arr) {
    const fieldOption = document.createElement('option');
    fieldOption.setAttribute('value', arr[i]);
    fieldOption.textContent = arr[i];
    frag.appendChild(fieldOption);
  }

  fieldFilter.append(frag);
...
[...labels].map((item, i) => {
  item.insertAdjacentElement(
    'beforeBegin',
    createSelector(filtersName[i], filtersNameArr[i], filtersAriaLabels[i]),
  );
  item.remove();
});
...
ymaps.ready(['Panel']).then(function() {
  ...
  var createPrevieClub = club => {
    return (
      ...
      </div><a class="card-club__pic" href=` + club.href + `><img src=` + club.poster + ` width="666" height="228px" alt="фитнес-клуб с&nbsp;большим бассейном"></a>
  ...
  console.log(createListFilterClubs());
  
  let modal = new bootstrap.Modal(document.querySelector('#modal-preview-club'));
  let openListClubs = document.querySelector('.clubs-search__sidebar');
  let customBalloon = document.querySelector(
  ...
  collection.events.add('click', function(e) {
    ...
    if (window.matchMedia('(min-width: 768px)').matches) {
      ...
      if (openListClubs.classList.contains('show')) {
        openListClubs.classList.remove('show');
      }
