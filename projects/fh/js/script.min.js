...
const coachesFormSearch = document.querySelector('.coaches-form-search');
const coachesFormSearchWrapper = document.querySelector(
  '.coaches-form-search .form__wrapper',
);
const btnMoreForm = document.querySelector(
  '.coaches-form-search__btn-more',
);

btnMoreForm &&
  btnMoreForm.addEventListener('click', () => {
    if (coachesFormSearch) {
      coachesFormSearchWrapper.classList.toggle('d-grid');
      if (coachesFormSearchWrapper.classList.contains('d-grid')) {
        btnMoreForm.innerHTML = 'Свернуть';
        btnMoreForm.classList.remove('button--outline');
      } else {
        btnMoreForm.innerHTML = 'Больше параметров';
        btnMoreForm.classList.add('button--outline');
      }
    }
  });
...
const btnCoachGallery = document.querySelector('.coach-achievements__coach');
const btnClientGallery = document.querySelector('.coach-achievements__client');
const btnVideoGallery = document.querySelector('.coach-achievements__video');
const videoBigCarousel = document.querySelector('.gallery-video-big');
const videoBigSlides = videoBigCarousel.querySelectorAll(
  '.swiper-slide.poster',
);
const videoBigBtnNext = videoBigCarousel.querySelectorAll(
  '.swiper-button-next',
);
const videoBigBtnPrev = videoBigCarousel.querySelectorAll(
  '.swiper-button-prev',
);

const strAutoPlay = 'rel=0&amp;autoplay=1';
const getStrAutoPlayCount = str => {
  return str.split('?').filter(x => x === strAutoPlay).length;
};

const activeVideo = activeSlide => {
  const img = activeSlide.querySelector('img');
  const iframe = activeSlide.querySelector('iframe');
  const srcIframe = iframe.getAttribute('src');
  img?.classList.add('d-none');
  getStrAutoPlayCount(srcIframe) === 0 && iframe.setAttribute('src', srcIframe + '?' + strAutoPlay);
};

const noActiveVideo = slide => {
  const img = slide.querySelector('img');
  const iframe = slide.querySelector('iframe');
  const sourceVideo = iframe.getAttribute('src');
  img?.classList.remove('d-none');
  if (getStrAutoPlayCount(sourceVideo) === 1) {
    sourceVideo.replace(strAutoPlay, '');
    iframe.setAttribute('src', sourceVideo);
  }
};

const galleryVideoThumbs = new Swiper('.gallery-video-thumbs', {
  slidesPerView: 3,
  autoHeight: true,
  loop: true,
  freeMode: true,
...
const galleryVideoTop = new Swiper('.gallery-video-big', {
  spaceBetween: 10,
  loop: true,
  navigation: {
  ...
  on: {
    slideChange: function() {
      [...videoBigSlides].map(slide => {
        noActiveVideo(slide);
      });
    },
    click: function() {
      [...videoBigSlides].map(slide => {
        slide.classList.contains('swiper-slide-active') && activeVideo(slide);
      });
    },
  },
});
...
const galleryTop = new Swiper('.gallery-big', {
  spaceBetween: 10,
  loop: true,
  navigation: {
    nextEl: '.swiper-button-next',
...
document.body.addEventListener('load', () => {
  ...
  galleryTop.destroy();
  galleryVideoThumbs.destroy();
  galleryVideoTop.destroy();
});

btnCoachGallery &&
  btnCoachGallery.addEventListener('click', () => {
    setTimeout(() => {
...
btnClientGallery &&
  btnClientGallery.addEventListener('click', () => {
    setTimeout(() => {
      galleryThumbs.update();
      galleryTop.update();
    }, 200);
...
btnVideoGallery &&
  btnVideoGallery.addEventListener('click', () => {
    setTimeout(() => {
      galleryVideoThumbs.update();
      galleryVideoTop.update();
    }, 200);
  });

const sliderReviews = new Swiper('#slider-coach-review', {
  spaceBetween: 10,
  navigation: {
    nextEl: '.swiper-button-next',
    prevEl: '.swiper-button-prev',
